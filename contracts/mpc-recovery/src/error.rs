pub const ONLY_OWNER: &str = "only owner";
pub const OWNER_IS_SELF: &str = "owner must not be this account, which ends with no keys";
pub const ONLY_INSTALLER: &str = "only installer or owner";
pub const ONLY_OWNER_REINSTALL: &str = "only owner may replace an existing policy";
pub const ONLY_TRANSFER_AUTHORITY: &str = "only transfer authority";
pub const NO_STATE: &str = "no contract state to migrate";
pub const BAD_THRESHOLD: &str = "threshold must be in 1..=watchers";
pub const NO_POLICY: &str = "no recovery policy for account";
pub const NOT_IDLE: &str = "recovery already in progress";
pub const COOLING_DOWN: &str = "a recovery request was recently aborted, try again shortly";
pub const NOT_REQUESTED: &str = "no recovery requested";
pub const NOT_APPROVED: &str = "recovery not approved";
pub const NOT_ACTIVE: &str = "no abortable recovery in progress";
pub const STALE_ROUND: &str = "stale or invalid round";
pub const ROUND_EXHAUSTED: &str = "recovery rounds exhausted";
pub const BAD_ATTESTATION: &str = "invalid attestation signature";
pub const TIMELOCK: &str = "timelock not elapsed";
pub const NO_QUORUM: &str = "watcher quorum not met";
pub const NO_REGISTRY: &str = "no registry configured for name recovery";
pub const REGISTRY_IS_SELF: &str = "registry must not be this account";
pub const QUORUM_EXPIRED: &str = "the watcher quorum for this recovery has expired";
pub const BAD_SIGNATURE_LEN: &str = "signature must be 64 bytes";
pub const TIMELOCK_TOO_SHORT: &str = "timelock below minimum";
pub const TIMELOCK_TOO_LONG: &str = "timelock above maximum";
pub const DUPLICATE_WATCHER: &str = "duplicate watcher key";
pub const TOO_MANY_WATCHERS: &str = "watcher set is full";
pub const MPC_NOT_ED25519: &str = "native mpc key must be ed25519";
pub const WATCHER_NOT_ED25519: &str = "watcher key must be ed25519";
pub const ATTESTATION_NOT_ED25519: &str = "attestation key must be ed25519";
pub const REQUIRES_ONE_YOCTO: &str = "requires an attached deposit of exactly 1 yoctoNEAR";
pub const EMPTY_CODE: &str = "code must not be empty";
pub const NO_APPROVED_HASH: &str = "no approved code hash";
pub const HASH_MISMATCH: &str = "code does not match the approved hash";
pub const APPROVAL_TOO_YOUNG: &str = "approved code must wait out the delay before upgrading";
pub const THRESHOLD_TOO_LOW: &str = "threshold must be at least 2";
pub const NAME_NOT_REQUESTED: &str = "no attested name recovery is pending";
pub const DESTINATION_CHANGED: &str = "quorum destination does not match the attested request";
pub const TIMELOCK_ACTIVE: &str = "timelock has not elapsed";
pub const INVALID_LEASED_ACCOUNT: &str = "tla and name do not form a valid account id";
pub const ONLY_INSTALLER_OR_HOLDER: &str = "only installer or owner or the account under recovery";
pub const NOT_ARMED: &str = "the account has not armed a policy install";
pub const ARMED_KEY_MISMATCH: &str = "attestation key does not match the armed install";
pub const ARMED_TIMELOCK_MISMATCH: &str = "timelock does not match the armed install";
pub const STATE_VERSION_UNKNOWN: &str = "state version is not the one this code understands";
pub const UPGRADE_NOT_PROVEN: &str = "the upgrade path has not been exercised on this account yet";
