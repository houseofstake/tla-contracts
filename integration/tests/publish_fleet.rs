...
use common::{account, wasm, GLOBAL_CODE_COST_PER_BYTE};
    ...
    for attempt in 0..4 {
        ...
        if let Ok(view) = worker.view(&id, "hos_lease").await {
            if let Ok(lease) = view.json::<serde_json::Value>() {
                if let Some(version) = lease.get("impl_version").and_then(|v| v.as_u64()) {
                    return Some(version);
                }
            }
        }
...
async fn publish_and_migrate_the_fleet() -> Result<()> {
    ...
    let council_id = account("COUNCIL_ACCOUNT");
    let council = Account::from_secret_key(council_id.parse()?, load_key(&council_id)?, &worker);
    let deployer_id: AccountId = account("DEPLOYER_ACCOUNT").parse()?;
    let extension_id: AccountId = account("EXTENSION_ACCOUNT").parse()?;
    ...
    if published.as_deref() == Some(hash.as_str()) {
        ...
        let payer_id = std::env::var("PUBLISH_PAYER").unwrap_or_else(|_| council_id.clone());
