...
async fn read_lease(worker: &Testnet, id: &AccountId) -> Option<serde_json::Value> {
    for attempt in 0..3 {
        if attempt > 0 {
            tokio::time::sleep(std::time::Duration::from_millis(400)).await;
        }
        if let Ok(view) = worker.view(id, "hos_lease").await {
            if let Ok(lease) = view.json::<serde_json::Value>() {
                return Some(lease);
            }
        }
    }
...
async fn layout_holds_for(worker: &Testnet, name: &str) -> Result<()> {
    ...
    let deployed_impl = lease
        .get("impl_version")
        .and_then(|v| v.as_u64())
        .with_context(|| format!("{name}: lease is missing impl_version"))?;
    let publishing = u64::from(hos_wallet::IMPL_VERSION);
    if deployed_impl >= publishing {
        bail!(
            "{name}: runs impl_version {deployed_impl} and the wasm about to be published \
             reports {publishing}. A publish must move the fleet forward; equal means the \
             code changed without bumping the version, which leaves nothing able to tell a \
             migrated account from one the migration missed."
        );
    }
...
async fn the_publish_is_affordable(worker: &Testnet) -> Result<()> {
    ...
    let payer_id = std::env::var("PUBLISH_PAYER").unwrap_or_else(|_| account("COUNCIL_ACCOUNT"));
...
async fn mint_preconditions_agree(worker: &Testnet) -> Result<()> {
    let registry: AccountId = account("REGISTRY_ACCOUNT").parse()?;
    let fees: serde_json::Value = worker.view(&registry, "get_fee_config").await?.json()?;
    let creation_deposit: u128 = fees
        .get("account_creation_deposit_yocto")
        .and_then(|v| v.as_str())
        .context("fee config carries no account_creation_deposit_yocto")?
        .parse()?;

    let tlas: Vec<serde_json::Value> = worker
        .view(&registry, "list_tlas")
        .args_json(json!({ "from_index": 0, "limit": 100 }))
        .await?
        .json()?;
    if tlas.is_empty() {
        bail!("the registry reports no TLAs, so the mint path cannot be checked");
    }
    ...
    for tla in &tlas {
        let id = tla
            .get("tla_id")
            .and_then(|v| v.as_str())
            .context("tla view carries no tla_id")?;
        let account: AccountId = id.parse()?;

        let min_balance: u128 = worker
            .view(&account, "min_balance")
            .await?
            .json::<String>()?
            .parse()?;
        if creation_deposit < min_balance {
            bail!(
                "{id} refuses a mint funded with less than {min_balance} yocto but the registry \
                 attaches {creation_deposit}. Every mint under this TLA would fail after the \
                 renter has paid."
            );
        }

        if worker.view(&account, "min_label_len").await.is_ok() {
            bail!(
                "{id} still answers min_label_len, so it runs a registrar build carrying a \
                 configurable label floor. Any value above one refuses names the registry has \
                 already charged for, which strands them as refunded and parked. Redeploy the \
                 registrar before opening minting."
            );
        }
    }
    println!("mint preconditions agree across {} TLAs", tlas.len());
    Ok(())
}
...
async fn the_fleet_is_safe_to_republish() -> Result<()> {
    ...
    mint_preconditions_agree(&worker).await?;
    ...
    for name in &names {
        let id: AccountId = name.parse()?;
        match read_lease(&worker, &id).await {
            Some(lease) => versions.push(
                lease
                    .get("impl_version")
                    .and_then(|v| v.as_u64())
                    .context("hos_lease without impl_version")?,
            ),
            None => unreadable.push(name.clone()),
        }
    }
