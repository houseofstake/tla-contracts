...
const IMPL_GAS_MARGIN: NearToken = NearToken::from_near(3);
...
async fn ensure_subaccount(
    ...
    let id: AccountId = format!("{label}.{}", root.id()).parse()?;
...
fn publish_delay_ns() -> u64 {
    std::env::var("PUBLISH_APPROVAL_DELAY_NS")
        .ok()
...
async fn deploy_demo_fleet_to_testnet() -> Result<()> {
    ...
    let root_key =
        load_key(&root_account).with_context(|| format!("{root_account} credentials not found"))?;
    let root_id: AccountId = root_account.parse()?;
    let root = Account::from_secret_key(root_id.clone(), root_key, &worker);
    println!(
        "root {root_account}: {}",
        balance_of(&worker, &root_id).await?
    );
    ...
    if is_live(&worker, &deployer_id).await {
        ...
        deployer
            ...
            .args_json(json!({
                "council": council.id(),
                "approval_delay_ns": U64(publish_delay_ns()),
            }))
            .max_gas()
            .transact()
            .await?
    ...
    if published_hash(&worker, &deployer_id).await.as_deref() == Some(wallet_hash.as_str()) {
        ...
        let floor = publish_cost.saturating_add(IMPL_GAS_MARGIN);
        let held = balance_of(&worker, &deployer_id).await?;
        if held < floor {
            let top_up = floor.saturating_sub(held);
    ...
    if registrar_live {
        println!("  code refreshed on {root_account}, state kept");
    } else {
        root.call(&root_id, "new")
            ...
            .await?
            .into_result()?;
        println!("  deployed registrar on {root_account}");
    }
    ...
    let tenant: AccountId = format!("{label}.{root_account}").parse()?;
...
async fn deploy_marketplace_to_testnet() -> Result<()> {
    ...
    let root = Account::from_secret_key(
        root_id.clone(),
        load_key(&root_account).context("root creds")?,
        &worker,
    );
    ...
    if has_code(&worker, recovery.id(), "threshold").await {
        ...
        if watchers.len() < 2 {
            bail!(
                "recovery needs at least two distinct watchers, got {}",
                watchers.len()
            );
        }
        let threshold: u32 = match std::env::var("RECOVERY_THRESHOLD") {
            Ok(value) => value
                .parse()
                .context("RECOVERY_THRESHOLD must be a number")?,
            Err(_) => 2,
        };
        recovery
        ...
        recovery
            ...
            .into_result()?;
        println!(
            "  deployed {} ({} watchers, threshold {threshold})",
            recovery.id(),
            watchers.len()
        );
    }
